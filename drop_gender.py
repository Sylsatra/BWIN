...
# We'll toggle whether we drop "2. What is your gender?".
...
###############################################################################
def run_experiment(drop_gender=False):
    """
    Returns:
    ...
    for seed in SEEDS:
        print(f"\n=== SEED={seed}, drop_gender={drop_gender} ===")
        np.random.seed(seed)
        ...
        # Optionally drop "gender"
        if drop_gender and ("2. What is your gender?" in df.columns):
            df.drop(columns=["2. What is your gender?"], inplace=True, errors='ignore')
        ...
        print(f"[Seed={seed}] SMOTE from {len(X_train)} to {len(X_res)} (train only). drop_gender={drop_gender}")
...
###############################################################################
def main():
    # 1) Run experiment WITHOUT dropping gender
    print("\n========== EXPERIMENT A: Keep Gender Column ===========\n")
    mlp_resA, sage_resA = run_experiment(drop_gender=False)
    mlp_meanA, mlp_stdA = mlp_resA
    sage_meanA, sage_stdA = sage_resA

    # 2) Run experiment WITH dropping gender
    print("\n========== EXPERIMENT B: Drop Gender Column ===========\n")
    mlp_resB, sage_resB = run_experiment(drop_gender=True)
    ...
    rows = [
        ["A (Keep Gender)", "MLP", f"{mlp_meanA:.4f}", f"± {mlp_stdA:.4f}"],
        ["A (Keep Gender)", "SAGE", f"{sage_meanA:.4f}", f"± {sage_stdA:.4f}"],
        ["B (Drop Gender)", "MLP", f"{mlp_meanB:.4f}", f"± {mlp_stdB:.4f}"],
        ["B (Drop Gender)", "SAGE", f"{sage_meanB:.4f}", f"± {sage_stdB:.4f}"],
    ]
    ...
    output_file = os.path.join(output_dir, "drop_gender_experiment.svg")
