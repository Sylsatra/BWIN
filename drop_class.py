...
# We'll toggle whether we drop "9. How often do you miss or skip classes due to sleep-related issues (e.g., insomnia, feeling tired)?".
TIMESTAMP_COL = "Timestamp"
CLASS_COL    = "9. How often do you miss or skip classes due to sleep-related issues (e.g., insomnia, feeling tired)?"
...
###############################################################################
def run_experiment(drop_class=False):
    """
    Returns:
    ...
    for seed in SEEDS:
        print(f"\n=== SEED={seed}, drop_class={drop_class} ===")
        np.random.seed(seed)
        ...
        # Optionally drop "class"
        if drop_class and ("9. How often do you miss or skip classes due to sleep-related issues (e.g., insomnia, feeling tired)?" in df.columns):
            df.drop(columns=["9. How often do you miss or skip classes due to sleep-related issues (e.g., insomnia, feeling tired)?"], inplace=True, errors='ignore')
        ...
        print(f"[Seed={seed}] SMOTE from {len(X_train)} to {len(X_res)} (train only). drop_class={drop_class}")
...
###############################################################################
def main():
    # 1) Run experiment WITHOUT dropping class
    print("\n========== EXPERIMENT A: Keep class Column ===========\n")
    mlp_resA, sage_resA = run_experiment(drop_class=False)
    mlp_meanA, mlp_stdA = mlp_resA
    sage_meanA, sage_stdA = sage_resA

    # 2) Run experiment WITH dropping class
    print("\n========== EXPERIMENT B: Drop class Column ===========\n")
    mlp_resB, sage_resB = run_experiment(drop_class=True)
    ...
    rows = [
        ["A (Keep class)", "MLP", f"{mlp_meanA:.4f}", f"± {mlp_stdA:.4f}"],
        ["A (Keep class)", "SAGE", f"{sage_meanA:.4f}", f"± {sage_stdA:.4f}"],
        ["B (Drop class)", "MLP", f"{mlp_meanB:.4f}", f"± {mlp_stdB:.4f}"],
        ["B (Drop class)", "SAGE", f"{sage_meanB:.4f}", f"± {sage_stdB:.4f}"],
    ]
    ...
    output_file = os.path.join(output_dir, "drop_class_experiment.svg")
