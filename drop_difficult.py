...
# We'll toggle whether we drop "3. How often do you have difficulty falling asleep at night?".
TIMESTAMP_COL = "Timestamp"
DIFFICULT_COL    = "3. How often do you have difficulty falling asleep at night?"
...
###############################################################################
def run_experiment(drop_difficult=False):
    """
    Returns:
    ...
    for seed in SEEDS:
        print(f"\n=== SEED={seed}, drop_difficult={drop_difficult} ===")
        np.random.seed(seed)
        ...
        # Optionally drop "difficult"
        if drop_difficult and ("3. How often do you have difficulty falling asleep at night?" in df.columns):
            df.drop(columns=["3. How often do you have difficulty falling asleep at night?"], inplace=True, errors='ignore')
        ...
        print(f"[Seed={seed}] SMOTE from {len(X_train)} to {len(X_res)} (train only). drop_difficult={drop_difficult}")
...
###############################################################################
def main():
    # 1) Run experiment WITHOUT dropping difficult
    print("\n========== EXPERIMENT A: Keep difficult Column ===========\n")
    mlp_resA, sage_resA = run_experiment(drop_difficult=False)
    mlp_meanA, mlp_stdA = mlp_resA
    sage_meanA, sage_stdA = sage_resA

    # 2) Run experiment WITH dropping difficult
    print("\n========== EXPERIMENT B: Drop difficult Column ===========\n")
    mlp_resB, sage_resB = run_experiment(drop_difficult=True)
    ...
    rows = [
        ["A (Keep difficult)", "MLP", f"{mlp_meanA:.4f}", f"± {mlp_stdA:.4f}"],
        ["A (Keep difficult)", "SAGE", f"{sage_meanA:.4f}", f"± {sage_stdA:.4f}"],
        ["B (Drop difficult)", "MLP", f"{mlp_meanB:.4f}", f"± {mlp_stdB:.4f}"],
        ["B (Drop difficult)", "SAGE", f"{sage_meanB:.4f}", f"± {sage_stdB:.4f}"],
    ]
    ...
    output_file = os.path.join(output_dir, "drop_difficult_experiment.svg")
