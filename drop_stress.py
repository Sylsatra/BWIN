...
# We'll toggle whether we drop "14. How would you describe your stress levels related to academic workload?".
...
###############################################################################
def run_experiment(drop_stress=False):
    """
    Returns:
    ...
    for seed in SEEDS:
        print(f"\n=== SEED={seed}, drop_stress={drop_stress} ===")
        np.random.seed(seed)
        ...
        # Optionally drop "stress"
        if drop_stress and ("14. How would you describe your stress levels related to academic workload?" in df.columns):
            df.drop(columns=["14. How would you describe your stress levels related to academic workload?"], inplace=True, errors='ignore')
        ...
        print(f"[Seed={seed}] SMOTE from {len(X_train)} to {len(X_res)} (train only). drop_stress={drop_stress}")
...
###############################################################################
def main():
    # 1) Run experiment WITHOUT dropping stress
    print("\n========== EXPERIMENT A: Keep Stress Column ===========\n")
    mlp_resA, sage_resA = run_experiment(drop_stress=False)
    mlp_meanA, mlp_stdA = mlp_resA
    sage_meanA, sage_stdA = sage_resA

    # 2) Run experiment WITH dropping stress
    print("\n========== EXPERIMENT B: Drop Stress Column ===========\n")
    mlp_resB, sage_resB = run_experiment(drop_stress=True)
    ...
    rows = [
        ["A (Keep Stress)", "MLP", f"{mlp_meanA:.4f}", f"± {mlp_stdA:.4f}"],
        ["A (Keep Stress)", "SAGE", f"{sage_meanA:.4f}", f"± {sage_stdA:.4f}"],
        ["B (Drop Stress)", "MLP", f"{mlp_meanB:.4f}", f"± {mlp_stdB:.4f}"],
        ["B (Drop Stress)", "SAGE", f"{sage_meanB:.4f}", f"± {sage_stdB:.4f}"],
    ]
    ...
    output_file = os.path.join(output_dir, "drop_stress_experiment.svg")
