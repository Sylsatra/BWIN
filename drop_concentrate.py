...
# We'll toggle whether we drop "7. How often do you experience difficulty concentrating during lectures or studying due to lack of sleep?".
TIMESTAMP_COL = "Timestamp"
CONCENTRATE_COL    = "7. How often do you experience difficulty concentrating during lectures or studying due to lack of sleep?"
...
###############################################################################
def run_experiment(drop_concentrate=False):
    """
    Returns:
      - average TestAcc for MLP
      - average TestAcc for GraphSAGE
    """
    ...
    for seed in SEEDS:
        print(f"\n=== SEED={seed}, drop_concentrate={drop_concentrate} ===")
        np.random.seed(seed)
        ...
        # Optionally drop "concentrate"
        if drop_concentrate and ("7. How often do you experience difficulty concentrating during lectures or studying due to lack of sleep?" in df.columns):
            df.drop(columns=["7. How often do you experience difficulty concentrating during lectures or studying due to lack of sleep?"], inplace=True, errors='ignore')
        ...
        print(f"[Seed={seed}] SMOTE from {len(X_train)} to {len(X_res)} (train only). drop_concentrate={drop_concentrate}")
...
###############################################################################
def main():
    # 1) Run experiment WITHOUT dropping concentrate
    print("\n========== EXPERIMENT A: Keep concentrate Column ===========\n")
    mlp_resA, sage_resA = run_experiment(drop_concentrate=False)
    mlp_meanA, mlp_stdA = mlp_resA
    sage_meanA, sage_stdA = sage_resA

    # 2) Run experiment WITH dropping concentrate
    print("\n========== EXPERIMENT B: Drop concentrate Column ===========\n")
    mlp_resB, sage_resB = run_experiment(drop_concentrate=True)
    ...
    rows = [
        ["A (Keep concentrate)", "MLP", f"{mlp_meanA:.4f}", f"± {mlp_stdA:.4f}"],
        ["A (Keep concentrate)", "SAGE", f"{sage_meanA:.4f}", f"± {sage_stdA:.4f}"],
        ["B (Drop concentrate)", "MLP", f"{mlp_meanB:.4f}", f"± {mlp_stdB:.4f}"],
        ["B (Drop concentrate)", "SAGE", f"{sage_meanB:.4f}", f"± {sage_stdB:.4f}"],
    ]
    ...
    output_file = os.path.join(output_dir, "drop_concentrate_experiment.svg")
