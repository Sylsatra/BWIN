...
# We'll toggle whether we drop "10. How would you describe the impact of insufficient sleep on your ability to complete assignments and meet deadlines?".
TIMESTAMP_COL = "Timestamp"
DEADLINE_COL    = "10. How would you describe the impact of insufficient sleep on your ability to complete assignments and meet deadlines?"
...
###############################################################################
def run_experiment(drop_deadline=False):
    """
    Returns:
    ...
    for seed in SEEDS:
        print(f"\n=== SEED={seed}, drop_deadline={drop_deadline} ===")
        np.random.seed(seed)
        ...
        # Optionally drop "deadline"
        if drop_deadline and ("10. How would you describe the impact of insufficient sleep on your ability to complete assignments and meet deadlines?" in df.columns):
            df.drop(columns=["10. How would you describe the impact of insufficient sleep on your ability to complete assignments and meet deadlines?"], inplace=True, errors='ignore')
        ...
        print(f"[Seed={seed}] SMOTE from {len(X_train)} to {len(X_res)} (train only). drop_deadline={drop_deadline}")
...
###############################################################################
def main():
    # 1) Run experiment WITHOUT dropping deadline
    print("\n========== EXPERIMENT A: Keep deadline Column ===========\n")
    mlp_resA, sage_resA = run_experiment(drop_deadline=False)
    mlp_meanA, mlp_stdA = mlp_resA
    sage_meanA, sage_stdA = sage_resA

    # 2) Run experiment WITH dropping deadline
    print("\n========== EXPERIMENT B: Drop deadline Column ===========\n")
    mlp_resB, sage_resB = run_experiment(drop_deadline=True)
    ...
    rows = [
        ["A (Keep deadline)", "MLP", f"{mlp_meanA:.4f}", f"± {mlp_stdA:.4f}"],
        ["A (Keep deadline)", "SAGE", f"{sage_meanA:.4f}", f"± {sage_stdA:.4f}"],
        ["B (Drop deadline)", "MLP", f"{mlp_meanB:.4f}", f"± {mlp_stdB:.4f}"],
        ["B (Drop deadline)", "SAGE", f"{sage_meanB:.4f}", f"± {sage_stdB:.4f}"],
    ]
    ...
    output_file = os.path.join(output_dir,"drop_deadline_experiment.svg")
