...
# We'll toggle whether we drop "5. How often do you wake up during the night and have trouble falling back asleep?".
TIMESTAMP_COL = "Timestamp"
WAKE_COL    = "5. How often do you wake up during the night and have trouble falling back asleep?"
...
###############################################################################
def run_experiment(drop_wake=False):
    """
    Returns:
    ...
    for seed in SEEDS:
        ...
        # Optionally drop "wake"
        if drop_wake and ("5. How often do you wake up during the night and have trouble falling back asleep?" in df.columns):
            df.drop(columns=["5. How often do you wake up during the night and have trouble falling back asleep?"], inplace=True, errors='ignore')
        ...
        print(f"[Seed={seed}] SMOTE from {len(X_train)} to {len(X_res)} (train only). drop_wake={drop_wake}")
...
###############################################################################
def main():
    # 1) Run experiment WITHOUT dropping wake
    print("\n========== EXPERIMENT A: Keep wake Column ===========\n")
    mlp_resA, sage_resA = run_experiment(drop_wake=False)
    mlp_meanA, mlp_stdA = mlp_resA
    sage_meanA, sage_stdA = sage_resA

    # 2) Run experiment WITH dropping wake
    print("\n========== EXPERIMENT B: Drop wake Column ===========\n")
    mlp_resB, sage_resB = run_experiment(drop_wake=True)
    ...
    rows = [
        ["A (Keep wake)", "MLP", f"{mlp_meanA:.4f}", f"± {mlp_stdA:.4f}"],
        ["A (Keep wake)", "SAGE", f"{sage_meanA:.4f}", f"± {sage_stdA:.4f}"],
        ["B (Drop wake)", "MLP", f"{mlp_meanB:.4f}", f"± {mlp_stdB:.4f}"],
        ["B (Drop wake)", "SAGE", f"{sage_meanB:.4f}", f"± {sage_stdB:.4f}"],
    ]
    ...
    output_file = os.path.join(output_dir, "drop_wake_experiment.svg")
