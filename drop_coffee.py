...
# We'll toggle whether we drop "12. How often do you consume caffeine (coffee, energy drinks) to stay awake or alert?".
TIMESTAMP_COL = "Timestamp"
COFFEE_COL    = "12. How often do you consume caffeine (coffee, energy drinks) to stay awake or alert?"
...
###############################################################################
def run_experiment(drop_coffee=False):
    """
    Returns:
    ...
    for seed in SEEDS:
        print(f"\n=== SEED={seed}, drop_coffee={drop_coffee} ===")
        np.random.seed(seed)
        ...
        # Optionally drop "coffee"
        if drop_coffee and ("12. How often do you consume caffeine (coffee, energy drinks) to stay awake or alert?" in df.columns):
            df.drop(columns=["12. How often do you consume caffeine (coffee, energy drinks) to stay awake or alert?"], inplace=True, errors='ignore')
        ...
        print(f"[Seed={seed}] SMOTE from {len(X_train)} to {len(X_res)} (train only). drop_coffee={drop_coffee}")
...
###############################################################################
def main():
    # 1) Run experiment WITHOUT dropping coffee
    print("\n========== EXPERIMENT A: Keep coffee Column ===========\n")
    mlp_resA, sage_resA = run_experiment(drop_coffee=False)
    mlp_meanA, mlp_stdA = mlp_resA
    sage_meanA, sage_stdA = sage_resA

    # 2) Run experiment WITH dropping coffee
    print("\n========== EXPERIMENT B: Drop coffee Column ===========\n")
    mlp_resB, sage_resB = run_experiment(drop_coffee=True)
    ...
    rows = [
        ["A (Keep coffee)", "MLP", f"{mlp_meanA:.4f}", f"± {mlp_stdA:.4f}"],
        ["A (Keep coffee)", "SAGE", f"{sage_meanA:.4f}", f"± {sage_stdA:.4f}"],
        ["B (Drop coffee)", "MLP", f"{mlp_meanB:.4f}", f"± {mlp_stdB:.4f}"],
        ["B (Drop coffee)", "SAGE", f"{sage_meanB:.4f}", f"± {sage_stdB:.4f}"],
    ]
    ...
    output_file = os.path.join(output_dir,"drop_coffee_experiment.svg")
