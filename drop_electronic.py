...
# We'll toggle whether we drop "11. How often do you use electronic devices (e.g., phone, computer) before going to sleep?".
TIMESTAMP_COL = "Timestamp"
ELECTRONIC_COL    = "11. How often do you use electronic devices (e.g., phone, computer) before going to sleep?"
...
###############################################################################
def run_experiment(drop_electronic=False):
    """
    Returns:
    ...
    for seed in SEEDS:
        print(f"\n=== SEED={seed}, drop_electronic={drop_electronic} ===")
        np.random.seed(seed)
        ...
        # Optionally drop "electronic"
        if drop_electronic and ("11. How often do you use electronic devices (e.g., phone, computer) before going to sleep?" in df.columns):
            df.drop(columns=["11. How often do you use electronic devices (e.g., phone, computer) before going to sleep?"], inplace=True, errors='ignore')
        ...
        print(f"[Seed={seed}] SMOTE from {len(X_train)} to {len(X_res)} (train only). drop_electronic={drop_electronic}")
...
###############################################################################
def main():
    # 1) Run experiment WITHOUT dropping electronic
    print("\n========== EXPERIMENT A: Keep electronic Column ===========\n")
    mlp_resA, sage_resA = run_experiment(drop_electronic=False)
    mlp_meanA, mlp_stdA = mlp_resA
    sage_meanA, sage_stdA = sage_resA

    # 2) Run experiment WITH dropping electronic
    print("\n========== EXPERIMENT B: Drop electronic Column ===========\n")
    mlp_resB, sage_resB = run_experiment(drop_electronic=True)
    ...
    rows = [
        ["A (Keep electronic)", "MLP", f"{mlp_meanA:.4f}", f"± {mlp_stdA:.4f}"],
        ["A (Keep electronic)", "SAGE", f"{sage_meanA:.4f}", f"± {sage_stdA:.4f}"],
        ["B (Drop electronic)", "MLP", f"{mlp_meanB:.4f}", f"± {mlp_stdB:.4f}"],
        ["B (Drop electronic)", "SAGE", f"{sage_meanB:.4f}", f"± {sage_stdB:.4f}"],
    ]
    ...
    output_file = os.path.join(output_dir,"drop_electronic_experiment.svg")
