...
# We always drop "Timestamp" as irrelevant
TIMESTAMP_COL  = "Timestamp"
# Now we toggle dropping the "13. How often do you engage in physical activity or exercise?"
PHYSICAL_COL   = "13. How often do you engage in physical activity or exercise?"
...
###############################################################################
# 6. RUN EXPERIMENT (keep or drop physical-activity column)
###############################################################################
def run_experiment(drop_physical=False):
    """
    ...
    for seed in SEEDS:
        print(f"\n=== SEED={seed}, drop_physical={drop_physical} ===")
        np.random.seed(seed)
        ...
        # Optionally drop physical-activity question
        if drop_physical and (PHYSICAL_COL in df.columns):
            df.drop(columns=[PHYSICAL_COL], inplace=True, errors='ignore')
        ...
        print(f"[Seed={seed}] SMOTE: from {len(X_train)} to {len(X_res)} (train). drop_physical={drop_physical}")
...
###############################################################################
def main():
    # Experiment A: KEEP the physical-activity column
    print("\n============= EXPERIMENT A: Keep Physical Activity Column =============\n")
    mlp_resA, sage_resA = run_experiment(drop_physical=False)
    mlp_meanA, mlp_stdA = mlp_resA
    sage_meanA, sage_stdA = sage_resA

    # Experiment D: DROP the physical-activity column
    print("\n============= EXPERIMENT B: Drop Physical Activity Column ============\n")
    mlp_resB, sage_resB = run_experiment(drop_physical=True)
    ...
    rows = [
        ["A (Keep Physical)", "MLP", f"{mlp_meanA:.4f}", f"± {mlp_stdA:.4f}"],
        ["A (Keep Physical)", "SAGE", f"{sage_meanA:.4f}", f"± {sage_stdA:.4f}"],
        ["D (Drop Physical)", "MLP", f"{mlp_meanB:.4f}", f"± {mlp_stdB:.4f}"],
        ["D (Drop Physical)", "SAGE", f"{sage_meanB:.4f}", f"± {sage_stdB:.4f}"],
    ]
    ...
    output_file = os.path.join(output_dir, "drop_physical_experiment.svg")
