...
# We'll always drop "Timestamp"
TIMESTAMP_COL  = "Timestamp"
# Now we'll toggle "6. How would you rate the overall quality of your sleep?"
...
###############################################################################
# 6. RUN EXPERIMENT (keep or drop "6. How would you rate the overall quality of your sleep?")
###############################################################################
def run_experiment(drop_sleep_quality=False):
    """
    Returns (mlp_mean, mlp_std), (sage_mean, sage_std)
    across multiple seeds, toggling whether to drop the SleepQuality col.
    """
    mlp_test_scores  = []
    sage_test_scores = []

    for seed in SEEDS:
        print(f"\n=== SEED={seed}, drop_sleep_quality={drop_sleep_quality} ===")
        np.random.seed(seed)
        ...
        # Optionally drop the "6. How would you rate the overall quality of your sleep?" column
        if drop_sleep_quality and (SLEEP_QUALITY_COL in df.columns):
            df.drop(columns=[SLEEP_QUALITY_COL], inplace=True, errors='ignore')
        ...
        print(f"[Seed={seed}] SMOTE: from {len(X_train)} to {len(X_res)}. drop_sleep_quality={drop_sleep_quality}")
...
###############################################################################
def main():
    # Experiment A: KEEP the sleep-quality column
    print("\n========== EXPERIMENT A: Keep Sleep Quality ===========\n")
    mlp_resA, sage_resA = run_experiment(drop_sleep_quality=False)
    mlp_meanA, mlp_stdA = mlp_resA
    sage_meanA, sage_stdA = sage_resA

    # Experiment B: DROP the sleep-quality column
    print("\n========== EXPERIMENT E: Drop Sleep Quality ===========\n")
    mlp_resB, sage_resB = run_experiment(drop_sleep_quality=True)
    mlp_meanB, mlp_stdB = mlp_resB
    sage_meanB, sage_stdB= sage_resB

    headers = ["Experiment", "Model", "Test Accuracy (Mean)", "Standard Deviation"]
    rows = [
        ["A (Keep Sleep Quality)", "MLP", f"{mlp_meanA:.4f}", f"± {mlp_stdA:.4f}"],
        ["A (Keep Sleep Quality)", "SAGE", f"{sage_meanA:.4f}", f"± {sage_stdA:.4f}"],
        ["E (Drop Sleep Quality)", "MLP", f"{mlp_meanB:.4f}", f"± {mlp_stdB:.4f}"],
        ["E (Drop Sleep Quality)", "SAGE", f"{sage_meanB:.4f}", f"± {sage_stdB:.4f}"],
    ]
    ...
    output_file = os.path.join(output_dir, "drop_sleep_quality_experiment.svg")
